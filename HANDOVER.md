...
- ガントチャート: start===endで表示されない問題を修正（最低1日差を保証）
- ガントチャート: 無効な日付のsafeDate処理を追加
- ガントチャート: 日/週/月の表示切替ボタンを追加
- サイドバー: レイアウトをh-screen+overflow-hiddenでメインのみスクロールに修正
- サイドバー: 折りたたみボタン追加
- フェーズプルダウン: タスク追加・編集モーダルでフェーズをプルダウン選択に変更
- タスク編集モーダル: 全項目をアプリ上で編集可能に
- オーナー非表示バッジ: グレー → 赤色に変更
- フェーズ色分け: タスク行の左ボーダー + フィルターボタンに色
- ユーザー事前登録: 店舗選択を任意に変更
...
| # | 内容 | 優先度 | 備考 |
|---|------|--------|------|
| 1 | フェーズ名の統一問題 | 高 | テンプレートCSVのフェーズ名と PHASE_DEFINITIONS のdateLabelが一致しないと色が付かない。PHASE_COLORSに使用するフェーズ名を追加するか、テンプレートCSVのフェーズ名を統一する必要あり |
| 2 | ダッシュボードの読み込み速度 | 高 | 全店舗のタスクを順次取得している。店舗数が増えると遅くなる。Cloud Functionsでサマリーをキャッシュするか、Firestoreのサブコレクションでカウンターを持つ方式に変更 |
| 3 | Googleカレンダーの重複登録防止 | 中 | 同じタスクを2回登録すると重複。eventIdをタスクに保存して重複チェック |
| 4 | テンプレート編集機能 | 中 | 現状は削除→再登録。インライン編集 or 編集モーダルが必要 |
| 5 | Slack通知の動作テスト | 中 | SLACK_WEBHOOK_URL/CRON_SECRET未設定。設定後に動作確認が必要 |
| 6 | Salesforce連携の動作テスト | 中 | SF側のフロー未作成。フロー作成後に動作確認が必要 |
| 7 | タスク一括削除機能 | 低 | フィルターしたタスクをまとめて削除 |
| 8 | オーナー向けメール/プッシュ通知 | 低 | 現状はSlackのみ |
| 9 | スマホ対応のガントチャート表示最適化 | 低 | 小さい画面での操作性改善 |
